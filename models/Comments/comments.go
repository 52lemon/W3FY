...
//添加评论 成功条件:(1)帖子存在
//insert into `comment`(`top_id`,`from_id`,`to_id`,`comments`,`created_at`)values(xxx,xxx,xxx,xxx)
...
// update `comment` set `is_deleted` =1,`comments`="评论已被折叠" where(`from_id`=id and select `is_deleted` from `topic` where(`id`=tid) = 0)
func DeleteComment(id int) bool {
	if err := models.DB.Debug().Model(&Comment{}).Where("id=?", id).Updates(map[string]interface{}{"is_deleted": 1, "comments": "评论已被折叠"}); err != nil {
		logging.DebugLog(err)
		return false
	}
	return true
